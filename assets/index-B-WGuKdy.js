(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=document.querySelector(`#app`),t={result:null,selectedTab:`output`};function n(e){try{return new Intl.DateTimeFormat(`ar-OM`,{dateStyle:`medium`,timeStyle:`short`}).format(new Date(e))}catch{return e}}function r(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#039;`)}function i(e){return e.replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`)}function a(e=``){let t=[],n=[],a=()=>{n.length&&(t.push(`<ul>${n.join(``)}</ul>`),n=[])};for(let o of r(e).split(`
`)){let e=o.trim();if(!e){a();continue}if(/^\d+[).،-]/.test(e)){n.push(`<li>${i(e.replace(/^\d+[).،-]\s*/,``))}</li>`);continue}if(/^[-•*]/.test(e)){n.push(`<li>${i(e.replace(/^[-•*]\s*/,``))}</li>`);continue}a(),t.push(`<p>${i(e)}</p>`)}return a(),t.join(``)}function o(e,t,n=``){return`<article class="metric-card">
    <span>${e}</span>
    <strong>${t}</strong>
    ${n?`<small>${n}</small>`:``}
  </article>`}function s(){if(!t.result){e.innerHTML=`<main class="shell loading"><div class="orb"></div><p>جاري تحميل نتيجة AI SDK...</p></main>`;return}let i=t.result,a=i.usage?`${i.usage.inputTokens??i.usage.promptTokens??`—`} in / ${i.usage.outputTokens??i.usage.completionTokens??`—`} out`:`غير متاح`;e.innerHTML=`
    <main class="shell">
      ...
      <section class="metrics">
        ${o(`وقت التوليد`,`${Math.round(i.durationMs/100)/10}s`,n(i.generatedAt))}
        ${o(`الاستخدام`,a,`من نتيجة AI SDK`)}
        ${o(`الحالة`,r(i.finishReason||`done`),`تم البناء والتحقق`)}
      </section>
  ...
  `,document.querySelectorAll(`[data-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{t.selectedTab=e.dataset.tab,s()})})}function c(e){return t.selectedTab===`proof`?`<h2>دليل أنها تجربة فعلية</h2>
      ...
      </ol>`:`<h2>الناتج المولّد لحسن</h2><div class="ai-output">${a(e.text)}</div>`}fetch(`./result.json`,{cache:`no-store`}).then(e=>{if(!e.ok)throw Error(`Failed to load result.json: ${e.status}`);return e.json()}).then(e=>{t.result=e,s()}).catch(t=>{e.innerHTML=`<main class="shell"><section class="panel glass"><h1>تعذر تحميل التجربة</h1><pre>${r(t.message)}</pre></section></main>`}),s();